...
const CartPage = () => {
    ...
    const onDelete = (cid) => {
        setBox({
            show:true,
            message: `${cid}번 장바구니를 삭제 하시겠습니까?`,
            action: async() => {
                await axios.post("/cart/delete", {cid:cid});
                getCart();
            }
        })
    };
    ...
    const onDeleteChecked = () => {
        if(count === 0){
            setBox({show:true, message:"삭제할 상품을 선택해 주세요!"});
        }else {
            setBox({
                show:true,
        ...
        <>
            {show==="cart" &&
                <div className='my-5'>
                    ...
                    <div className='mb-2'>
                        <Button variant="danger" size="sm" onClick={onDeleteChecked}>선택상품 삭제</Button></div>
                    ...
                    {books.length > 0 &&
                        ...
                        <Button variant='warning' className='px-5 ms-2'>쇼핑 계속하기</Button>
                    </div>
                </div>
            }
